...
function Logger(key, options) {
    ...
    this.key = key;
    this.source = {
        hostname: options.hostname || (!isBrowser ? os.hostname() : window.document.domain)
...
Logger.prototype._flush = function(cb) {
    if ((!isBrowser && !this._req) || this._buf.length === 0) {
    ...
    if (!isBrowser) {
        ...
        headers.set('Authorization', 'Basic ' + Buffer.from(this.key + ":" + "").toString('base64'));
        headers.set('Content-Type', 'application/json; charset=UTF-8');
        window.fetch(url, {
            method: 'POST',
            headers: headers,
            body: data
//            credentials: this.options.with_credentials || configs.REQUEST_WITH_CREDENTIALS
        });
    }
};
